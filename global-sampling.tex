...
From a dynamical perspective, which also applies to Monte Carlo data, how long must one wait before the system completely loses memory of its prior configuration?  \textcolor{red}{PNP comment: I left a question about this section in the corresponding issue thread.}
...
Two methods applicable for a single trajectory have been previously introduced by some of the authors, exploiting the fact that \textcolor{red}{trajectories are often correlated in time}.
...
Lyman and Zuckerman proposed a global ``decorrelation'' analysis by mapping a trajectory to a discretization of configuration space and analyzing the resulting statistics \cite{Lyman2007a}.  \textcolor{red}{Would be helpful to clarify what this configuration space is.  Am I incorrect in assuming it is the full 6N-dimensional phase-space?  Other comments embedded in the .tex file as commented lines.}
...
%Again, I have no idea what this means.  This problem could potentially be solved by definining the RMSD ideas a few sections up where they are first introduced.
...
%Is reference to the multinomial distribution necessary?  I find myself distracted by that word, wanting to know why that appears in the discretized phase-space, and why it's relevant for MD.
...
These methods are implemented as part of LOOS \cite{LOOS,LOOS-JCC}.
...
Employing a fairly simple kinetic-clustering technique to automatically define states, the approach then uses the variances in state populations among trajectories to estimate the effective sample size.  \textcolor{red}{What is the effective sample size?  Number of independent trajectory frames?}
