...
Briefly, the set of $N$ observations $\left\{x_1, ..., x_N\right\}$ are converted to a set of $M$ ``block averages'' $\left\{x^b_1, ..., x^b_{M}\right\}$, where a block average $x^b_k$ is the \hyperref[def:arith_mean]{arithmetic mean} of $n$ (the block size) sequential measurements of $x$:
%
\begin{equation}
  x^b_k = \dfrac{\sum\limits_{j=1+(k-1)n}^{kn} x_k}{n}
\end{equation}
%
From this set of block averages, one may then compute the arithmetic mean of the block averages, $\bar{x}^b$, which is an estimator for $\left<x\right>$.
(We note that for $mod(N,n)=0$, then $\bar{x}^b = \bar{x}$, as the entire set of $x_k$ is used. This is {\it not} true if $N$ is not evenly divisible by either $M$ or $n$.)
Following, one computes the \hyperref[def:exp_st_dev]{experimental standard deviation} of the block averages, $s\left(\bar{x}^b\right)$ using Eq.~\ref{def:exp_st_dev}.
Lastly, the standard uncertainty of $\bar{x}^b$ is just the \hyperref[def:exp_st_dev_mean]{experimental standard deviation of the mean} given the set of $M$ block averages:
%
\begin{equation}
  s\left(\bar{x}^b\right) = \dfrac{s\left(x^b_k\right)}{\sqrt{M}}
\end{equation}
%
This standard uncertainty may then be used to calculate a confidence interval on $\bar{x}^b$.

It is crucial when performing this analysis to do so systematically as a function of block size; as the blocks get longer, the block averages should decorrelate and $s\left(\bar{x}^b\right)$ should plateau \cite{Flyvbjerg-1989,Grossfield2009}.
...
First, given a set of observations $\left\{x_1, ..., x_N\right\}$, the autocorrelation function $C$ is computed for a set of lags $j$:
%
\begin{equation}
  C_j = \dfrac{
    ...
    %\left< \left[ x_{k{\color{white}j}}\hspace{-1pt} - \bar{x} \right]
    %  \left[ x_{k+j} - \bar{x} \right]
    %\right>
    \overline{
      \left( x_{k{\color{white}j}}\hspace{-1pt} - \bar{x} \right)
      \left( x_{k+j} - \bar{x} \right)
    }
  }
  {s\left(x_k\right)^2}
...
where the denominator is the square of the \hyperref[def:exp_st_dev]{experimental standard deviation} of $x_k$ given by Eq.~\ref{def:exp_st_dev}.
...
%
\begin{equation}
  s\left(\bar{x}\right) = \dfrac{s\left(x_k\right)}{\sqrt{N_{ind}}}
  \label{def:ACF_std_unc}
\end{equation}
%
We note that the \hyperref[def:exp_st_dev]{experimental standard deviation} of the observable $x$ is used in Eq.~\ref{def:ACF_std_unc} to estimate the uncertainty. Strictly speaking, the standard uncertainty should be estimated using the true standard deviation of $x$ (e.g.\ $\sigma_x$); given that the true standard deviation is unknown, the experimental standard deviation is used in its place as an {\it unbiased estimate} of $\sigma_x$ {\color{red}(GOING TO NEED A CITATION HERE!!!)}.
