...
\begin{itemize}
\item For observables that are approximately Gaussian-distributed (or assumed to be, due to lack of information), an appropriately chosen \emph{coverage factor} $k$ (typically about 2, but see Sec.~\ref{sec:conf_int}) multiplying the standard uncertainty $\stdunc$ by $k$ yields an estimate for a 95~\% confidence interval.
...
First, given a set of observations $\left\{q_1, ..., q_N\right\}$, the autocorrelation function $C$ is computed for a set of lags $j$:
%
\begin{equation}
  C_j = \dfrac{
