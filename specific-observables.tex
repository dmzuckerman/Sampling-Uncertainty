...
\item For non-Gaussian observables, a \emph{bootstrapping} approach (Sec.~\ref{sec:bootstrap}) should be used.
  ...
  Bootstrapping (or block averaging) is also sometimes necessary for estimating uncertainties associated with \hyperref[def:deriv_obs]{derived observables}; this is true for quantities such as time correlation functions and anything derived from them.  \textcolor{red}{PNP comment: I'm not sure this last statement is actually true.  Any reference to support this?  I don't even know how one would do bootstrap with time correlation functions, since order of data-points matters a lot.}
\end{itemize}
