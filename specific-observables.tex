...
(We note that for $mod(N,n)=0$, then $\mean{x}^b = \mean{x}$, as the entire set of $x_k$ is used. This is {\it not} true if $N$ is not evenly divisible by either $M$ or $n$.)
...
%
\begin{equation}
  \stdevmean{\mean{x}^b} = \dfrac{\stdev{x^b_k}}{\sqrt{M}}
\end{equation}
%
...
%
\begin{equation}
  \stdevmean{\mean{x}} = \dfrac{\stdev{x_k}}{\sqrt{N_{ind}}}
