...
\begin{itemize}
\item For observables that are approximately Gaussian-distributed, an appropriately chosen \emph{coverage factor} $k$ (typically about 2, but see below) multiplying the standard uncertainty $\stdunc$ yields an estimate for a 95~\% confidence interval.
...
Below we describe approaches for estimating the standard uncertainty $s\left(\bar{q}\right)$ and coverage factor $k$ from a single trajectory, as well as discussing the bootstrapping approach for direct confidence-interval estimation.
...
\subsection{Block averaging method for estimating the standard uncertainty}
...
The block averaging technique can be used instead of the individual observations to avoid correlation in samples, while using all data, subject only to the constraint that the ``blocks'' are large enough to be essentially uncorrelated\cite{Friedberg1970,Flyvbjerg-1989,FrenkelSmit2002,Grossfield2009}.

Briefly, the set of $N$ observations $\left\{q_1, ..., q_N\right\}$ are converted to a set of $M$ ``block averages'' $\left\{q^b_1, ..., q^b_{M}\right\}$, where a block average $q^b_k$ is the \hyperref[def:arith_mean]{arithmetic mean} of $n$ (the block size) sequential measurements of $q$:
%
\begin{equation}
  q^b_k = \dfrac{\sum\limits_{j=1+(k-1)n}^{kn} q_k}{n}
\end{equation}
%
From this set of block averages, one may then compute the arithmetic mean of the block averages, $\bar{q}^b$, which is an estimator for $\left<q\right>$.
(We note that for $mod(N,n)=0$, then $\bar{q}^b = \bar{q}$, as the entire set of $q_k$ is used. This is {\it not} true if $N$ is not evenly divisible by either $M$ or $n$.)
Following, one computes the \hyperref[def:exp_st_dev]{experimental standard deviation} of the block averages, $s\left(\bar{q}^b\right)$ using Eq.~\ref{def:exp_st_dev}.
Lastly, the standard uncertainty of $\bar{q}^b$ is just the \hyperref[def:exp_st_dev_mean]{experimental standard deviation of the mean} given the set of $M$ block averages:
%
\begin{equation}
  s\left(\bar{q}^b\right) = \dfrac{s\left(q^b_k\right)}{\sqrt{M}}
\end{equation}
%
This standard uncertainty may then be used to calculate a confidence interval on $\bar{q}^b$.

It is crucial when performing this analysis to do so systematically as a function of block size; as the blocks get longer, the block averages should decorrelate and $s\left(\bar{q}^b\right)$ should plateau \cite{Flyvbjerg-1989,Grossfield2009}.
...
\subsection{Autocorrelation method for estimating the standard uncertainty}
...
First, given a set of observations $\left\{q_1, ..., q_N\right\}$, the autocorrelation function $C$ is computed for a set of lag times $j$:
%
\begin{equation}
  C_j = \dfrac{
    % The weird white "j" and negative hspace is to make the square brackets the same size
    % Is $\langle \rangle>$ the best way to express the mean in the ACF?
    \left< \left[ q_{k{\color{white}j}}\hspace{-1pt} - \bar{q} \right] 
      \left[ q_{k+j} - \bar{q} \right]
    \right>
  }
  {s\left(q_k\right)^2}
\end{equation}
%
{\color{red}NOTE: Have to decide how to express an average here; we reserved $\left< \cdots \right>$ for true expectation values.}

where the denominator is the square of the \hyperref[def:exp_st_dev]{experimental standard deviation} of $q_k$ given by Eq.~\ref{def:exp_st_dev}.
Then, the number of independent samples is estimated by\footnote{{\color{red} Explain why this sum is not the same as the integration over $C$; $C$ is stationary and symmetric, 1 comes from the ``zeroth'' lag, 2 is for symmetry. Actually easy to see this if you do an integral from -infty to +infty by a midpoint integration.}}
%
\begin{equation}
...
%
\begin{equation}
  s\left(\bar{q}\right) = \dfrac{s\left(q_k\right)}{\sqrt{N_{ind}}}
  \label{def:ACF_std_unc}
\end{equation}
%
We note that the \hyperref[def:exp_st_dev]{experimental standard deviation} of the observable $q$ is used in Eq.~\ref{def:ACF_std_unc} to estimate the uncertainty. Strictly speaking, the standard uncertainty should be estimated using the true standard deviation of $q$ (e.g.\ $\sigma_q$); given that the true standard deviation is unknown, the experimental standard deviation is used in its place as an {\it unbiased estimate} of $\sigma_q$ {\color{red}(GOING TO NEED A CITATION HERE!!!)}.
...
\begin{table}
    \begin{tabular}{S S}
      ...
      \bottomrule
    \end{tabular}
  \caption{Coverage factors $k$ required for a 95~\% confidence interval for a Gaussian variable \cite{JCGM:GUM2008}.}
