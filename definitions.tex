...
We particularly suggest readers study the term ``standard uncertainty'' which is often estimated by (in common parlance) the ``standard error of the mean''; however, the VIM term for the latter is the ``experimental standard deviation of the mean.''
...
We strongly encourage reading through the full glossary because of its structure and potentially unfamiliar terminology.

In the section following the glossary, we briefly describe the motivation for the modern terminology, which otherwise may be confusing.
...
\smallskip
\textbf{\textit{Remark:}} ``Experimental'' here and below can mean simulated data, which is considered to result from a numerical experiment -- in contrast to analytical results derived mathematically.
...
\textbf{\textit{Remark:}} The experimental standard deviation of the mean, familiarly known as the ``standard error of the mean'' characterizes the dispersion of the arithmetic mean relative to its expectation.
...
In cases where data is approximately normally distributed, the standard uncertainty is computed as the experimental standard deviation of the mean.
...
\smallskip
\textbf{\textit{Remark:}} Independence of two variables means that the value of one variable does not affect distribution of the other.
...
\item {\bf Two-sided confidence interval}: A statistically derived pair of lower and upper bounds between which the (true) expectation value of a random quantity is likely to fall, as quantified by a {\it confidence level} (typically given as a percentage, e.g., 95~\%). For Gaussian-distributed data, the confidence level, in conjunction with the sample size, determines the {\it coverage factor} which multiplies the experimental standard deviation of the mean \hyperref[def:exp_st_dev_mean]{$s\left(\bar{q}\right)$} to assign the bounds of the confidence interval.
...
Rather, statistical analyses are better interpreted as quantifying our state of knowledge, e.g., our ability to confidently state that the ``true'' mean is within some interval.
