...
For one, time-series data generally displays a certain amount of correlation (or even autocorrelation) in the sense that nearby points in the series tend to cluster close to one another.  Intuition dictates that correlated data does not reveal ``new'' information about the quantity-of-interest, so that we require (linearly) uncorrelated samples to achieve meaningful sampling \cite{PatroneAIAA}.\footnote{This intuition is actually somewhat misguided in the sense that anti-correlated data actually {\it increases} our knowledge of a given random quantity.  See, for example, the discussion in Ref.~\cite{PatroneAIAA}.}  Thus, it is important to ask: what are the pertinent timescales of the system?  
...
In addition to autocorrelation analyses (CITE RELAVANT SECTION), one general strategy that will allow you to understand the relevant timescales in a system is to perform several repeats of the same simulation protocol: as described below, repeats can be used to assess variance in \emph{any} observable, within the time you have run your simulation.
